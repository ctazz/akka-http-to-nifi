...
//NOTE:
//The as[IpPairSummaryRequest] directive and the Unmarshal(response.entity).to[IpInfo] conversion
// work because Service extends Protocols. Protocols brings in
//standard spray-json   Case class <=> JSON conversions.
  ...
  val routes: Route = {
    logRequestResult("akka-http-microservice") {
      pathPrefix("ip") {
        (get & path(Segment)) { ip =>
         logger.info("hey!!!!!!!")
          logger.info(s"ip is " + ip)
          //println("ip xxx is $ip")
          complete {
